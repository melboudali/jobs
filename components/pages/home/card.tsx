...
interface Props {
	children?: ReactNode;
	data: { logo: string; name: string; title: string; price?: string; time?: string; place?: string };
	testimonial: boolean;
}
...
const Card = ({ data: { logo, name, title, price, time, place }, testimonial, children }: Props) => {
	const root = cls(styles.root, { [styles["testimonial"]]: testimonial, [styles["job"]]: !testimonial });
	return (
		<div className={root}>
